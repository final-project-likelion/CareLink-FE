...
function ChatBubble({ msg }) {
  ...
  return (
    <div className={`m-[25px] flex items-start ${isUser ? 'justify-end' : 'justify-start'}`}>
      ...
      <div
        className={`max-w-[42%] bg-[#EAEAEA] text-[18px] px-[15px] py-5 ${isUser ? 'rounded-tr-[10px] rounded-bl-[10px] rounded-tl-[10px]' : 'rounded-tr-[10px] rounded-bl-[10px] rounded-br-[10px]'}`}
      >
        {msg.text}
      </div>
    </div>
